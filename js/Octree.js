...
class Octree {
    ...
    triangleCapsuleIntersect(capsule, triangle) {
        ...
        const capsulePoint = _v1.copy(capsule.start).lerp(capsule.end, 0.5);
        const planeNormal = _plane.normal.clone();
        /*const distanceToPoint = _plane.distanceToPoint(capsulePoint);
        if (distanceToPoint > 0) {
            planeNormal.negate();
            planeNormal.normalize();
        }*/
        const dprod = capsulePoint.clone().sub(triangle.closestPointToPoint(capsulePoint, new Vector3())).normalize().dot(planeNormal);
        if (dprod < 0) {
            planeNormal.negate();
            planeNormal.normalize();
        }
        if (triangle.containsPoint(intersectPoint)) {
            return { normal: planeNormal, point: intersectPoint.clone(), depth: Math.abs(Math.min(d1, d2)) };
        }
